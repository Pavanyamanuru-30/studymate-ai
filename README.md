...
## ?? Intermediate Level Feature: Document Q&A (RAG)
We upgraded the application to meet the Intermediate Level requirements by implementing a Retrieval-Augmented Generation (RAG) pipeline:
- **PDF Ingestion:** Users can upload PDF documents.
- **Vector Database:** Utilizes FAISS to store embeddings generated by Google's gemini-embedding-2 model.
- **Grounded Generation:** The LLM strictly answers questions based on the retrieved document context, preventing hallucinations.
- **Source Citations:** Displays the exact retrieved chunks used to generate the answer.
